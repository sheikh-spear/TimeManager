      ...
      if (worktimes && worktimes.length > 0) {
        ...
        if (this.mode == "weekly") {
          ...
          let mondayDate = new Date(
            this.date.getTime() - this.date.getDay() * 24 * 60 * 60 * 1000
          );
          ...
          let sundayDate = new Date(
            this.date.getTime() + (6 - this.date.getDay()) * 24 * 60 * 60 * 1000
          );
          ...
          worktimes.forEach(dt => {
            ...
            let hours =
              (dt.end.getTime() - dt.start.getTime()) / 1000 / 60 / 60;
            let totalHours = daysHours.get(day)
              ? daysHours.get(day) + hours
              : hours;
            daysHours.set(day, totalHours);
          });

          for (let i = 0; i < 7; i++) {
            let hours = (daysHours.get(i) ? daysHours.get(i) : 0).toPrecision(
              3
            );
          ...
          title = "Worktime from ";
          title +=
            (mondayDate.getDate() > 10 ? "" : "0") + mondayDate.getDate() + "/";
          title +=
            (mondayDate.getMonth() > 10 ? "" : "0") +
            mondayDate.getMonth() +
            "/";
          title += mondayDate.getFullYear() + " to ";
          title +=
            (sundayDate.getDate() > 10 ? "" : "0") + sundayDate.getDate() + "/";
          title +=
            (sundayDate.getMonth() > 10 ? "" : "0") +
            sundayDate.getMonth() +
            "/";
          ...
          worktimes = worktimes.filter(
            dt =>
              dt.start.getDay() == this.date.getDay() &&
              dt.start.getMonth() == this.date.getMonth() &&
              dt.start.getFullYear() == this.date.getFullYear()
          );
          ...
          worktimes.forEach(dt => {
            let hours = (
              (dt.end.getTime() - dt.start.getTime()) /
              1000 /
              60 /
              60
            ).toPrecision(3);
            //Format to HH:mm
            labels.push(
              `${dt.start.getHours() < 10 ? "0" : ""}${dt.start.getHours()}h${
                dt.start.getMinutes() < 10 ? "0" : ""
              }${dt.start.getMinutes()}`
            );
            data.push(hours);
          });
